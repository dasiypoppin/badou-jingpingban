...
#         super(TorchModel, self).__init__()
#         self.embedding = nn.Embedding(len(vocab), vector_dim)  #embedding层
#         self.RNN = nn.RNN(vector_dim, hidden_size)   #池化层
...
#     def forward(self, x, y=None):
#         x = self.embedding(x)                      #(batch_size, sen_len) -> (batch_size, sen_len, vector_dim)
#         x = self.RNN(x)                           #(batch_size, sen_len, vector_dim) -> (batch_size, vector_dim)
#         x = self.classify(x)                      #(batch_size, vector_dim) -> (batch_size, 1)
#         y_pred = self.activation(x)                #(batch_size, 1) -> (batch_size, 1)
#         if y is not None:
#             return self.loss(y_pred, y)   #预测值和真实值计算损失
...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab, hidden_size):
        super(TorchModel, self).__init__()
        self.embedding = nn.Embedding(len(vocab), vector_dim)  #embedding层
        self.RNN = nn.RNN(vector_dim, hidden_size, batch_first=True)   #池化层
        self.classify = nn.Linear(hidden_size, 1)     #线性层
        self.activation = torch.sigmoid     #sigmoid归一化函数
        self.loss = nn.functional.mse_loss  #loss函数采用均方差损失
    ...
    def forward(self, x, y=None):
        ...
        x = x[:, -1, :]                            #(batch_size, sen_len, hidden_size) -> (batch_size, hidden_size)
